...
class ProductView(View):
	def get(self, request):
		totalitem = 0
		xiomi = Product.objects.filter(category='X')
		realme = Product.objects.filter(category='R')
		samsung = Product.objects.filter(category='S')
		if request.user.is_authenticated:
			totalitem = len(Cart.objects.filter(user=request.user))
		return render(request, 'app/home.html', {'xiomi':xiomi, 'realme':realme, 'samsung':samsung, 'totalitem':totalitem})
